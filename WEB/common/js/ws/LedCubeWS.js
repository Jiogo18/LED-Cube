...
export class LedCubeWS extends EventEmitter {
	...
	constructor() {
		...
		if (document.location.pathname.endsWith('/login/')) {
			var comeFromLogin = this.cookies.getItem('comeFromLogin');
			if (comeFromLogin) {
				comeFromLogin = parseInt(comeFromLogin) || 1;
				this.cookies.setItem('comeFromLogin', comeFromLogin + 1);
			...
			this.cookies.removeItem('comeFromLogin');
		}
	}
	...
	connect() {
		if (!this.cookies.areAllowed()) {
			return; // Cookies are required to connect
		}
	...
	tryCookieConnect() {
		...
		var auth = this.cookies.getAuth();
		var cookieKey = auth.ck;
		var clientChallenge = auth.cc;
